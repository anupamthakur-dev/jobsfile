...
export default function CollapsibleItems({ nav }: {nav:CollapsibleInput}) {
  ...
  <NavLink to={nav.route}>
    {({ isActive }) => (
      <SidebarMenuButton isActive={isActive} className="font-semibold">
        <Icon iconName={nav.icon}/><span>{nav.title}</span>
      </SidebarMenuButton>
    )}
  </NavLink>
) : (
  <SidebarMenuButton className="font-semibold">
   <Icon iconName={nav.icon}/><span>{nav.title}</span>
      ...
      {nav.items ?<CollapsibleContent>
          <SidebarMenuSub>
            {
             (isSideNavJobRouteArray(nav.items) && nav.items.length===0 )   && <SidebarMenuSubItem className="text-xs italic text-gray-400">no jobs yet</SidebarMenuSubItem>
            }
            {isSideNavJobRouteArray(nav.items) &&
              nav.items.map((i:jobRoute) => (
                <SidebarMenuSubItem key={i.id}>
                <NavLink to={`${i.id}`}>
                    {({isActive})=><SidebarMenuSubButton isActive={isActive}><Icon iconName={i.icon} /><span className="text-sm">{i.name}</span></SidebarMenuSubButton>}
